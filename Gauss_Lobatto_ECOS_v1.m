...
th_g_deg = rad2deg(th_g);

% Calling variables at collocation points
r_c = paraTRAJ.x_g(:,1); w_c = paraTRAJ.x_g(:,2); vr_c = paraTRAJ.x_g(:,3);
vth_c = paraTRAJ.x_g(:,4); vw_c = paraTRAJ.x_g(:,5); t_c = paraTRAJ.x_g(:,6);
z_c = paraTRAJ.x_g(:,7); taur_c = paraTRAJ.x_g(:,8); tauth_c = paraTRAJ.x_g(:,9);
...
% Cartesian plot
[x_cart] = transform_state_cyl_to_cart(paraTRAJ.x_g, th_g');
...
r_rec = paraTRAJ.x_g_rec(:,1); w_rec = paraTRAJ.x_g_rec(:,2); 
vr_rec = paraTRAJ.x_g_rec(:,3); vth_rec = paraTRAJ.x_g_rec(:,4); 
vw_rec = paraTRAJ.x_g_rec(:,5); t_rec = paraTRAJ.x_g_rec(:,6); 
z_rec = paraTRAJ.x_g_rec(:,7);

% Variabvles errors as functions of ToF
err_r = abs(r_c - r_rec);
err_w = abs(w_c - w_rec);
err_vr = abs(vr_c - vr_rec);
err_vth = abs(vth_c - vth_rec);
err_vw = abs(vw_c - vw_rec);
err_t = abs(t_c - t_rec);
err_z = abs(z_c - z_rec);

% Errors w.r.t the final boundary conditions
r_bound = abs(r_rec(end) - paraSCP.xf(end,1))
w_bound = abs(w_rec(end) - paraSCP.xf(end,2))
vr_bound = abs(vr_rec(end) - paraSCP.xf(end,3))
vth_bound = abs(vth_rec(end) - paraSCP.xf(end,4))
vw_bound = abs(vw_rec(end) - paraSCP.xf(end,5))
